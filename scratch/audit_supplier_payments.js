...
async function runAudit() {
  ...
  const { data: payments } = await supabase
    ...
    .select('*, sembako_suppliers(*)')
    .eq('tenant_id', tenantId)
    .eq('is_deleted', false)

  console.log('--- ALL SUPPLIER PAYMENTS ---')
  payments.forEach((p, idx) => {
    console.log(`Payment #${idx+1}: ID=${p.id} | Amount=${p.amount} | Date=${p.payment_date} | Supplier=${p.sembako_suppliers?.supplier_name} (ID: ${p.supplier_id})`)
  })
}
runAudit()
