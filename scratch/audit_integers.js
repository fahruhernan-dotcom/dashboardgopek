...
function processSaleRowLocal(sale, returnsData = [], itemsBySaleId = {}) {
  ...
  // Use computed net profit if there are returns, otherwise default to DB net_profit
...
async function runAudit() {
  ...
  processedSales.forEach((s, idx) => {
    ...
    // Calculate realized & unrealized with integer rounding
    const ratio = s.total_amount > 0 ? Math.max(0, Math.min(1, s.paid_amount / s.total_amount)) : 1
    const realized = Math.round(s.net_profit * ratio)
    const unrealized = s.net_profit - realized
    
    totalRealizedProfit += realized
    totalUnrealizedProfit += unrealized

    console.log(`Sale #${idx + 1}: ID=${s.id.slice(0,6)} | Total=${s.total_amount} | Paid=${s.paid_amount} | NetProfit=${s.net_profit} | Realized=${realized} | Unrealized=${unrealized}`)
  })

  console.log('\n--- AGGREGATE TOTALS ---')
  console.log(`Sum of Revenue:             Rp${totalRevenue.toLocaleString()}`)
  console.log(`Sum of Net Profit:          Rp${totalNetProfit.toLocaleString()}`)
  console.log(`Sum of Piutang:             Rp${totalOutstanding.toLocaleString()}`)
  console.log(`Sum of Paid Amount:         Rp${totalPaidAmount.toLocaleString()}`)
  console.log(`Sum of Realized Profit:     Rp${totalRealizedProfit.toLocaleString()}`)
  console.log(`Sum of Unrealized Profit:   Rp${totalUnrealizedProfit.toLocaleString()}`)
  console.log(`Realized + Unrealized Sum:  Rp${(totalRealizedProfit + totalUnrealizedProfit).toLocaleString()}`)
}
runAudit()
