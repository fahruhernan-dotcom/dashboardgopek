    ...
    queryFn: async () => {
      try {
        ...
        return (data || []).map(sale => {
          ...
          // Hanya hitung uang MASUK (positif, bukan refund). Refund ditrack terpisah.
          ...
          // Hitung total uang yang sudah dikembalikan ke toko (refund)
          ...
          // total_amount = subtotal sebelum retur, dikurangi retur
          // otherCost TIDAK ditambah karena sale.total_amount dari DB sudah mencakupnya
          const total_amount = Math.max(0, initialSubtotal - totalReturnAmount)

          // raw_paid: jumlah bersih uang masuk dari toko setelah dikurangi refund yang sudah dikembalikan
          ...
          return {
            ...sale,
            items,
            subtotal: initialSubtotal,   // gross sebelum retur — untuk display di detail sheet
            delivery_cost: deliveryCost,
            other_cost: otherCost,
            total_cogs: totalCogs,
            net_profit,
            gross_profit: grossProfit,
            total_amount,                // net setelah retur — sumber kebenaran
            paid_amount,
    ...
    mutationFn: async (id) => {
      ...
      try {
        ...
        if (saved) {
          ...
          localStorage.setItem('gopek_retur_list', JSON.stringify(filtered))
        }
      } catch (e) {
